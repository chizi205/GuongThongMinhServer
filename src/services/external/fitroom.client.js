...
class FitroomClient {
  ...
  async _getImageStream(urlOrPath) {
    if (urlOrPath.startsWith("http://") || urlOrPath.startsWith("https://")) {
      const response = await axios.get(urlOrPath, { responseType: "stream" });
   ...
   * @param {string} modelImagePath - Đường dẫn file ảnh người mẫu (local path hoặc URL)
   * @param {string} clothImagePath - Đường dẫn file ảnh trang phục (local path hoặc URL)
  ...
  async createTryOnTask(
    ...
    try {
      ...
      const modelStream = await this._getImageStream(modelImagePath);
      const clothStream = await this._getImageStream(clothImagePath);

      formData.append("model_image", modelStream, "model.jpg");
      formData.append("cloth_image", clothStream, "cloth.jpg");
