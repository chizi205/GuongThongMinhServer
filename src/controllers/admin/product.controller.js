...
const createProduct = async (req, res) => {
  try {
    const { shop_id, name, category_id, description, status, base_price } = req.body;
    const finalShopId = shop_id || req.user?.shop_id;

    if (!finalShopId) {
      return res.status(400).json({ success: false, message: "Thiếu thông tin Cửa hàng (shop_id)" });
    }
    ...
    // 2. TẠO SLUG DUY NHẤT
    const slug = name ? generateSlug(name) + "-" + Date.now().toString().slice(-5) : null;
...
const createVariant = async (req, res) => {
  try {
    ...
    let { sku, size, color, price, stock, measurements } = req.body;

    // Nếu measurements gửi từ FormData sẽ là string, cần parse sang Object/Array
    ...
    const newVariant = await productService.createVariant({
      ...
      stock,
      model_3d_url,
      measurements, // Truyền thông số đo xuống service
    });
...
const updateVariant = async (req, res) => {
  try {
    ...
    const updated = await productService.updateVariant(variantId, {
      ...
      stock,
      model_3d_url,
      measurements, // Cập nhật cả thông số đo
    });
